...
__global__ void kerDot(T* oper1, T* oper2, T* res, int N) {
    // TODO: THIS DOESN'T WORK!!!!!!! this needs to have
    // a temporary array to store the multiplication results,
    // and all the values have to be summed via prefix sum.
    unsigned int tid = blockDim.x*blockIdx.x + threadIdx.x;

    if (tid >= N) {
        return;
    }
    *res += oper2[tid] * oper1[tid];
}
...
template<typename T>
T VecN<T>::deviceDot(VecN<T>* vec2) {

    cudaError_t lastError = cudaGetLastError();

    if (this->N != vec2->N) {
        throw std::runtime_error(
                "[ERROR, VecN::deviceDot()]:  Cannot dot vectors of mismatched length!"
                );
    }
    ...
    cudaMalloc(&devOper1, vecBytes);
    cudaMalloc(&devOper2, vecBytes);
    cudaMalloc(&devRes, sizeof(T));

    lastError = cudaGetLastError();
    if (lastError != cudaSuccess) {
        throw std::runtime_error("[ERROR, VecN::deviceDot()]: Failed to allocate memory!");
    }
    ...
    lastError = cudaGetLastError();
    if (lastError != cudaSuccess) {
        throw std::runtime_error("[ERROR, VecN::deviceDot()]: Failed to transfer host memory to device!");
    }
    ...
    kerDot<<<gridDim, blockDim>>>(devOper1, devOper2, devRes, N);

    // transfer devRes to host.
    T* sum = (T*) malloc(sizeof(T));
    cudaMemcpy(sum, devRes, sizeof(T), cudaMemcpyDeviceToHost);
    ...
    lastError = cudaGetLastError();
    if (lastError != cudaSuccess) {
        throw std::runtime_error("[ERROR, VecN::deviceDot()]: Failed to free device memory!");
    }
...
template __global__ void kerDot(float*, float*, float*, int);
