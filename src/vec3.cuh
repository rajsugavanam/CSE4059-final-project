...
class Vec3 {
   public:
    ...
    // Accessors
    __host__ __device__ float x() const { return e_[0]; }
    __host__ __device__ float y() const { return e_[1]; }
    __host__ __device__ float z() const { return e_[2]; }
    ...
    __host__ __device__ float operator[](int idx) const { return e_[idx]; }

    __host__ __device__ float& operator[](int idx) { return e_[idx]; }
...
inline __host__ std::ostream& operator<<(std::ostream& os, const Vec3& vec) {
...
inline __host__ __device__ Vec3 operator*(float t, const Vec3& v) {
    return Vec3(t * v.e_[0], t * v.e_[1], t * v.e_[2]);
}

inline __host__ __device__ Vec3 operator*(const Vec3& v, float t) {
    return t * v;
}
...
inline __host__ __device__ float dot(const Vec3& u, const Vec3& v) {
